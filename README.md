...
每个子项目解决一个具体问题（卡死 / 上下文隔离），并存留一份可用备份，保证：
...
**Prereq / 前置**: Node.js 18+, opencode (`npm install -g opencode-ai`).
...
Edit `opencode.jsonc` (location per OS / 路径按系统):
...
Append to `plugin` array (absolute paths / 用绝对路径):
加到 `plugin` 数组 (绝对路径):
...
### 3. Restart + verify / 重启 + 验证

Restart opencode, then run / 重启 opencode, 然后跑:

```bash
# Windows
powershell -ExecutionPolicy Bypass -File scripts/check-watchdog.ps1

# Linux / macOS
bash scripts/check-watchdog.sh
```

Output `ACTIVE` = success / 输出 `ACTIVE` = 成功。
...
### 4.2 task-context-injector — 子代理上下文注入
...
## 5. opencode 升级 / 换机后的恢复清单

### 5.1 opencode 普通升级（配置一般保留，插件路径不变）
...
- ✅ `ACTIVE` → 一切正常，无需任何操作。
- ❌ `FAIL: watchdog.log not found` → 检查 §5.1-a / §5.1-b：
  - **5.1-a 版本破坏插件 API**（升级后 event 名 / hook 签名变更）→ 更新插件适配新事件 schema（先看 `%TEMP%\watchdog.log` 是否还有 `EVENT` 行，以及 opencode changelog）。
  - **5.1-b 配置被重置/覆盖** → 用 `config-backup/opencode.jsonc` 为底，填回实际 API key 与路径，覆盖权威源，重启。
...
# 4. 按 §1 注册两个插件 → 重启 → check-watchdog.ps1 验证 ACTIVE
```
